...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  // 🔍 Query corregida - Busca por categoría ignorando mayúsculas/minúsculas
  let articles: any[] = [];
  try {
    articles = await client.fetch(
      `*[_type == "news" && (category == $category || category == $categoryCap || lower(category) == lower($category)) && defined(slug.current)] | order(_createdAt desc) {
        _id,
        title,
        "slug": slug.current,
        category,
      ...
      { 
        ...
        categoryCap: category.name // Busca también "Política"
      }
    );
    
    console.log(`✅ Noticias encontradas en "${category.name}":`, articles.length);
  } catch (error) {
