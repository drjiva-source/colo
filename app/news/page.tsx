...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: article.description,
      url: `/news/${slug}`,
      images: [{ url: article.image || "/default-og.jpg", width: 1200, height: 630, alt: article.title }],
      type: "article",
  ...
  return (
    <article className="max-w-4xl mx-auto px-4 py-12">
      ...
      <nav className="text-sm text-muted mb-6 flex items-center gap-2">
        ...
        <span>/</span>
        <Link href={`/categoria/${article.category}`} className="hover:text-primary transition">
          {article.category}
        </Link>
        <span>/</span>
      ...
      <div className="relative w-full h-[400px] md:h-[500px] mb-8 rounded-2xl overflow-hidden bg-muted/20">
        {article.image ? (
          <Image
            src={article.image}
            alt={article.title}
          ...
          <div className="w-full h-full flex items-center justify-center text-muted bg-muted/20">
            Sin imagen
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-4 py-6 border-y border-border mb-12">
        ...
        <a href="#" className="p-2 bg-card border border-border rounded hover:bg-primary hover:border-primary hover:text-primary-foreground transition">
